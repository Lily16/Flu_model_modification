...
In this vignette, we will work with only one risk group as we noticed in the vignette `First solution school closure` that school closure had the same effects for low risk and high risk people. 
...
population <- age_groups_school_closure

```

## Initial infected

The number of people infected at the beginning of the season, again separated into age groups.
...
cov2<-NULL
cov2 <- coverage[,c("Under.65","Under.65","Under.65","Under.65","Under.65","Under.65","X65")]/100.0
names(cov2)[1]<- "0-2"
names(cov2)[2]<- "3-5"
names(cov2)[3]<- "6-12"
names(cov2)[4]<- "13-16"
...
vaccine_calendar_s <- as_vaccination_calendar(efficacy = c(0.7, 0.7, 0.7,0.7, 0.7, 0.7,0.4), dates = coverage$Date,coverage = cov2, no_age_groups = 7, no_risk_groups =1)


#Vaccination calendar for pandemic flu 

cov2p <- NULL
cov2p <- rep(c(0,0,0,0,0,0,0),19)
...
vaccine_calendar_p <- as_vaccination_calendar(
  ...
  dates = coverage$Date,coverage = cov2p, no_age_groups = 7, no_risk_groups =1)
...
infection_odin_closure <- function(population, initial_infected, vaccine_calendar,contact_matrix_school,contact_matrix_closure, susceptibility, transmissibility, infection_delays, interval, date_begin_closure = 0, date_end_closure = 0) {
  ...
  no_age_groups <- ncol(contact_matrix)
  no_risk_groups <- 1
  ...
  mod <- gen_seeiir_ag_vacc_closure(no_groups = no_groups, cij_nc = contact_matrix_school,cij_c = contact_matrix_closure, trans = transmissibility,
                            pop = population,
                            I0 = initial_infected,
...
We can conclude that the total number of people has been reduced by between 9% and 53%. 
